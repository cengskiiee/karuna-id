...
<!-- Start Row -->
    <div class="row">
            ...
            <div class="card" style="border-left: 5px solid blue; background-color: rgba(0, 0, 255, 0.1);">
                <div class="card-body">
                    ...
                    <p style="display: none;" class="card-title-desc font-size-16 content-toggle">
                        The <span class="fw-bold text-primary text-justify">Activities Content</span> table is a tool designed to help you manage and customize the activities content displayed on the website. This table allows you to add new content, edit existing ones, and remove those that are no longer needed. Each entry can have a title, an image, and a description in both Indonesian and English.
                    </p>
                </div>
            </div>
        </div>
        <!-- End Info -->
                ...
                <div class="col-lg-12">
                    <div class="card">
                        <div class="card-body">
                            <h4 class="my-3 fw-bold">Activities Content Table</h4>
                            <div class="d-flex justify-content-between align-items-center">
                                <button type="button" class="btn btn-primary px-4 py-2" data-bs-toggle="modal" data-bs-target="#activitiesModal" onclick="resetForm()">Add Activities</button>
                            <!-- Container -->
                            <div class="d-flex align-items-center gap-3">
                                ...
                                <div class="d-flex align-items-end mt-1 gap-1">
                                   ...
                                   <select id="perPage" class="form-select" name="per_page" onchange="updatePerPage()">
                                       ...
                                       <option value="{{ $activities->total() }}" {{ $perPage == $activities->total() ? 'selected' : '' }}>All</option>
                                   </select>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="d-flex gap-3">
                                                <a class="btn btn-primary btn-sm" title="Detail" onclick="showActivities('{{ $data->slug }}')">
                                                    <i class="fa fa-eye"></i>
                                                </a>
                                                <a class="btn btn-warning btn-sm" title="Edit" onclick="editActivities('{{ $data->slug }}')">
                                                    <i class="fas fa-pencil-alt"></i>
                                                </a>
                                                <a class="btn btn-danger btn-sm" title="Delete" onclick="deleteActivities('{{ $data->slug }}')">
                            ...
                            <div class="pagination-container mt-3">
                                {{ $activities->appends(['per_page' => $perPage, 'sort' => $sort])->links('pagination::bootstrap-5') }}
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <!-- Modal -->
        <div class="col-lg-12">
            <div class="modal fade" id="activitiesModal" tabindex="-1" aria-labelledby="edactivitiesdalLabel" aria-hidden="true">
                <div class="modal-dialog modal-xl">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="edactivitiesdalLabel">Add Activities</h5>
    ...
    <script>
        ...
        function resetForm() {
        $('#edactivitiesdalLabel').text('Add Activities');
    ...
    var url = id ? '/activities-content/' + id : '/activities-content';
...
function editActivities(slug) {
    $.get('/activities-content/' + slug, function(data) {
        $('#edactivitiesdalLabel').text('Edit Activities');
...
function showActivities(slug) {
    $.get('/activities-content/' + slug, function(data) {
        Swal.fire({
            title: '<h3>Activities Detail</h3>',
            icon: 'info',
            html: `
    ...
    function deleteActivities(slug) {
        Swal.fire({
            title: 'Are you sure?',
